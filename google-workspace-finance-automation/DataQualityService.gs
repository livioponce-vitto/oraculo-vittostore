var VITTO_DATA_QUALITY_VERSION_20260508 = 'VITTO_DATA_QUALITY_VERSION_20260508';
...
var DataQualityService = (function () {
  ...
  function runWeeklyDataQualitySummary() {
    ...
    AuditService.logInfo('Resumen semanal calidad procesado', JSON.stringify({
  ...
  function safeSendEmail_(payload, contextLabel) {
    try {
      ...
      AuditService.logWarn('MailApp no autorizado; envio omitido', JSON.stringify(detail));
  ...
  function runLedgerDataQualityChecks() {
    ...
    if (errorCount > 0) {
      var emailResult = sendCriticalFindingsEmail(summary, findings.filter(function (item) {
        return item[3] === 'ERROR';
      }));

      if (emailResult && !emailResult.sent) {
        AuditService.logWarn('Alerta por correo omitida en control calidad', JSON.stringify({
...
function probeDataQualityRuntime_20260508() {
  var report = {
