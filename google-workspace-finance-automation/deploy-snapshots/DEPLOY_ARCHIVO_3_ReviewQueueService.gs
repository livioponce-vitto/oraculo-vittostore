var ReviewQueueService = (function () {
  ...
  function buildRejectedKey(rowIndex) {
    return 'REJ-' + rowIndex;
  }
  ...
  function syncRejectedRecordsToReviewQueue() {
    ...
    rows.forEach(function (row, index) {
      var rejectedKey = buildRejectedKey(index + 2);
      if (existingKeys[rejectedKey]) {
        return;
      }

      queueSheet.appendRow(buildQueueRow(rejectedKey, row));
      refreshOperationalFields(queueSheet, queueSheet.getLastRow(), buildQueueRow(rejectedKey, row));
